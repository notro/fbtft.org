...
# -->
      ...
      if uri.scheme
        instance[:src] = download_image(site_source, uri)
      end
# <--
...
# -->
    def download_image(site_source, uri)
#      puts "download_image(site_source = #{site_source}, uri= #{uri})\n"
      fn = URI.split(uri.to_s).drop(1).join ""
      dl_base_dir = File.join(site_source, "_images")
      dl_dest_dir = File.join(dl_base_dir, File.dirname(fn))
      dl_dest_file = File.join(dl_base_dir, fn)

      unless File.exists?(dl_dest_file)
        FileUtils.mkdir_p(dl_dest_dir) unless File.exist?(dl_dest_dir)
        puts "Downloading #{uri}"
        File.open(dl_dest_file, "wb") do |saved_file|
